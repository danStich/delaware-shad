...
vb_data = list(
  ...
  x = as.numeric(as.factor(fish$rps)),
  p_linf = 0,
  p_linf_sd = 1,
  p_k = 0,
  p_k_sd = 1,  
...
rps_fit <- stan(file = 'models/vonbert_hmv_cov.stan',
                      data = vb_data,
                      chains = 3,
                      iter = 500,
                      warmup = 200,
...
new_rps <- unique(fish$rps)
new_rps_scaled <- as.vector(unique(scale(fish$rps)))

# Second, make predictions
#  .. Across both sexes ----
linf_mat <- matrix(0, nrow = length(pars$b0_linf), ncol = length(new_rps))
k_mat <- matrix(0, nrow = length(pars$b0_k), ncol = length(new_rps))
t0_mat <- matrix(0, nrow = length(pars$b0_t0), ncol = length(new_rps))
...
for(i in 1:nrow(linf_mat)){
  for(t in 1:ncol(linf_mat)){
    ...
    linf_mat[i, t] <- exp(
      mean(pars$Gamma[i, 1, ]) + 
        pars$b0_linf[i] + pars$ba_linf[i] * new_rps_scaled[t]
    )
    
    k_mat[i, t] <- exp(
      mean(pars$Gamma[i, 2, ]) + 
        pars$b0_k[i] + pars$ba_k[i] * new_rps_scaled[t]
    )
    
    t0_mat[i, t] <- exp(
      mean(pars$Gamma[i, 3, ]) + 
        pars$b0_t0[i] + pars$ba_t0[i] * new_rps_scaled[t]
    ) - 10
...
ggplot(preds, aes(x = RPS, y = fit)) +
  geom_line() +
  geom_point(data = fish_summary, aes(x = rps, y = fit)) +
  geom_segment(data = fish_summary, 
               aes(x = rps, xend = rps, y = lwr, yend = upr)) +
  geom_ribbon(
    aes(xmax = RPS, ymin = lwr, ymax = upr, color = NULL),
    alpha = 0.10) +
