...
vb_data = list(
  ...
  age = fish$final_age,
  nobs = nrow(fish),
  group = as.numeric(as.factor(fish$live_dead)),
  ngroups = length(unique(fish$live_dead)),
  hp_tau = 1,
  hp_sigma = 10,
  hp_omega = 4,
  p_b = 0,
...
fish$live_dead <- as.numeric(as.factor(fish$live_dead))
